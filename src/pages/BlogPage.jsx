        ...
        return (
            <div className="py-14 md:py-24 text-center mt-[80px]">
                <div className="max-w-7xl mx-auto px-5 md:px-8">
                    ...
                    <p className="text-text-muted">{t('blogPage.loading')}</p>
                </div>
            </div>
        );
    }
        ...
        <div className="min-h-screen">
            <section className="relative min-h-[400px] pt-[80px] flex items-center justify-center text-center text-white bg-cover bg-center" style={{
                ...
                <div className="max-w-7xl mx-auto px-5 md:px-8 text-center text-white">
                    <h1 className="text-4xl md:text-5xl font-bold font-heading mb-4">{t('blogPage.title')}</h1>
                    <p className="text-lg opacity-90 max-w-2xl mx-auto">{t('blogPage.subtitle')}</p>
                </div>
            </section>
            ...
            <div className="py-14 md:py-24 bg-surface">
                <div className="max-w-7xl mx-auto px-5 md:px-8">
                    {posts.length === 0 ? (
                        <div className="text-center text-lg text-text-muted font-medium">{t('blogPage.noPosts')}</div>
                    ) : (
                                ...
                                <article key={post.id} className="bg-white rounded-2xl overflow-hidden shadow-md hover:-translate-y-1.5 hover:shadow-xl transition-all duration-300 flex flex-col h-full group">
                                    ...
                                    <div className="p-6 flex flex-col flex-1">
                                        <div className="flex items-center justify-between text-sm text-text-muted mb-4">
                                            ...
                                            <span className="font-medium text-xs">{post.created_at ? new Date(post.created_at).toLocaleDateString() : t('blog.recent')}</span>
                                        </div>
                                        ...
                                        <Link to={`/blog/${post.slug}`} className="inline-flex items-center gap-2 text-primary font-semibold hover:text-secondary transition-colors mt-auto">{t('blogPage.readArticle')} &rarr;</Link>
                                    </div>
                                </article>
                            ))}
                        </div>
